...
@JsonController("/admin/clinic-certificate")
export class AdminClinicCertController {

    @Post()
    @OnUndefined(400)
    async create(@Req() request: Request, @Res() response: Response) {
        const token = request.get('token')
        const data = Deserialize(ClinicCert, request.body)
    ...
    async deactivate(@Req() request: Request, @Res() response: Response) {
        ...
        if (token != null && data.validate()) {
            const result = await AdminClinicService.deactivate(ClinicCert, token, data.id)
            return response.status(result.code).send(result.data)
        }
    }
